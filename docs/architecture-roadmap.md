...
- remote-control domain contracts、connection/screen/voice application policies、screen capture、
  WebRTC infrastructure 与 platform gateway → `lib/features/remote_control/`
...
remote-control socket owner、socket connection helpers 与 presentation，以及 browser/video
继续按下列顺序渐进处理。
