...
jq -e '
  ...
  and (["telegram", "webrtc_voice", "easytier", "life_runtime"]
    | all(. as $id | $root.plugins[$id] != null))
  and (
    $root.plugins
    | to_entries
    | all(
        .key as $id
        | .value as $plugin
        | ($plugin.versionCode > 0)
        and ($plugin.apiVersion > 0)
        and (
          (if $id == "life_runtime" then ["arm64-v8a"]
           else ["arm64-v8a", "armeabi-v7a"] end)
          | all(
              . as $abi
              | ($plugin.artifacts[$abi].size > 0)
              and ($plugin.artifacts[$abi].sha256 | test("^[0-9a-f]{64}$"))
              and ($plugin.artifacts[$abi].url | startswith("https://github.com/"))
            )
        )
      )
  )
